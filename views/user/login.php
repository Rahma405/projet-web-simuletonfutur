...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['reset_choice'])) {
        ...
        if ($choice === 'yes' && $emailForReset !== '') {
            ...
            if ($token !== null) {
                $resetLink = Config::getPublicBaseUrl() . '/public/index.php?route=/reset-password&token=' . urlencode($token);
    ...
    if (empty($erreurs) && $captchaRequired) {
        ...
        if ($stage === 1) {
            ...
            if ($captchaMath === '') {
                ...
                header('Location: ' . $baseUrl . '/public/index.php?route=/login');
                exit;
            }
        }
    }
    ...
    if (empty($erreurs) && $captchaUnlocked) {
        ...
        if ($utilisateur) {
            ...
            // Redirect to intended page if set, else default by role
            // Always go to homepage (dashboard) — it shows the full site after login
            if (isset($_SESSION['intended']) && $_SESSION['intended'] !== '') {
                $intended = $_SESSION['intended'];
                unset($_SESSION['intended']);
                header('Location: ' . $baseUrl . '/public/' . $intended);
            } else {
                header('Location: ' . $baseUrl . '/public/index.php?route=/');
            }
            exit;
    ...
    <form method="POST" novalidate>
                  ...
                  <?php for ($index = 0; $index < 9; $index++): ?>
                    <label class="captcha-tile">
                      ...
                      <span class="captcha-tile-frame">
                        <img
                          src="<?= htmlspecialchars($baseUrl . '/public/img/captcha/' . $captchaThemes[$captchaData['target']]['folder'] . '/tile-' . $index . '.jpg') ?>"
      ...
      <?php if (!$captchaRequired || $captchaStage === 1): ?>
        <button type="submit" class="btn-login">
          ...
          <a href="<?= $baseUrl ?>/public/index.php?route=/face-login" class="btn btn-outline-primary ms-2" style="border-radius:14px;padding:14px 18px;font-weight:700;">
      ...
      <div class="login-register">
        ...
        <a href="<?= $baseUrl ?>/public/index.php?route=/register"><?= htmlspecialchars(stf_t('create_account')) ?></a>
      </div>
    </form>
    <?php endif; ?>
  </div>
