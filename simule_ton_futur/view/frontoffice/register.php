...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ── Validation PHP uniquement (pas HTML5) ────────────────
    $erreursU = $ctrlU->valider($old, 0, true);
    $erreursP = $ctrlP->valider(array_merge($old, ['idUtilisateur' => 1])); // id factice pour validation
    unset($erreursP['idUtilisateur']); // pas requis ici
    $erreurs  = array_merge($erreursU, $erreursP);

    if (empty($erreurs)) {
        // 1. Créer l'utilisateur
        $u = new Utilisateur(null, $old['nom'], $old['prenom'], $old['email'], $old['motDePasse'], 'user');
        // Récupérer l'ID inséré
        $db = Config::getConnexion();
        $q  = $db->prepare("INSERT INTO utilisateur (nom,prenom,email,motDePasse,role)
                             VALUES (:nom,:prenom,:email,:mdp,'user')");
        $q->execute([
            ':nom'    => $u->getNom(),
            ':prenom' => $u->getPrenom(),
            ':email'  => $u->getEmail(),
        ...
        $p = new Profil(null, $old['bio'], 'default.png', $old['ville'], $old['pays'], $old['langue'], $newId);
        $ctrlP->addProfil($p);

        $_SESSION['message'] = [
            'type'  => 'success',
            'texte' => "🎉 Bienvenue {$old['prenom']} ! Ton profil a été créé avec succès.",
        ];
  ...
  <div class="row justify-content-center">
    <div class="col-lg-7">
      ...
      <div class="text-center mb-4">
        ...
        <h2 class="fw-bold" style="color:#1d2b4f">Créer mon compte</h2>
        <p class="text-muted" style="font-size:.88rem">Rejoins la communauté Simule Ton Futur</p>
      </div>
        ...
        <div class="card-body p-4">
          ...
          <form method="POST" novalidate>
            ...
            <div class="p-3 rounded mb-4" style="background:#f0f3fa;border-left:4px solid #e63946;">
              <div class="fw-semibold mb-3" style="font-size:.84rem;color:#1d2b4f">
                <i class="fas fa-id-card me-2" style="color:#e63946"></i>Informations du profil (optionnel)
              </div>
              ...
              <div class="mb-3">
                ...
                <textarea name="bio" rows="3"
                  ...
                  placeholder="Parle de toi en quelques mots… (max 500 caractères)"><?= htmlspecialchars($old['bio'] ?? '') ?></textarea>
            ...
            <button type="submit" class="btn-hero w-100" style="border-radius:10px;padding:13px;">
              <i class="fas fa-rocket me-2"></i>Créer mon compte
            </button>
            <div class="text-center mt-3">
              <a href="../../index.php" style="color:#8899bb;font-size:.8rem;text-decoration:none;">
