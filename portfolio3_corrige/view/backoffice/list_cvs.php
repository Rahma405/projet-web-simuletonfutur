  ...
  <div class="card-body p-4">
    ...
    <div class="table-responsive">
      <table class="table table-hover">
        ...
        <tr>
          <td style="color:#8899bb;font-size:.78rem"><?= $cv->getId() ?></td>
          <td>
            <div class="d-flex align-items-center gap-2">
              <div class="avatar"><?= strtoupper(substr($cv->getTitrePoste(), 0, 2)) ?></div>
              <span class="fw-semibold" style="font-size:.86rem"><?= htmlspecialchars($cv->getTitrePoste()) ?></span>
            </div>
          </td>
          <td style="font-size:.83rem"><?= htmlspecialchars($cv->getEmail()) ?></td>
          <td style="font-size:.83rem"><?= htmlspecialchars($cv->getTelephone()) ?></td>
          <td class="text-center">
            <div class="d-flex gap-1 justify-content-center">
              <a href="show_cv.php?id=<?= $cv->getId() ?>" class="btn btn-sm btn-outline-secondary" title="Voir"><i class="fas fa-eye"></i></a>
              <a href="edit_cv.php?id=<?= $cv->getId() ?>" class="btn btn-sm btn-outline-primary" title="Modifier"><i class="fas fa-edit"></i></a>
              <a href="delete_cv.php?id=<?= $cv->getId() ?>" class="btn btn-sm btn-outline-danger btn-delete" title="Supprimer"><i class="fas fa-trash"></i></a>
            </div>
          </td>
        </tr>
        <?php endforeach; ?>
