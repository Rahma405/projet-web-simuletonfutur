      ...
      <div class="col-md-6 col-lg-4">
        <div class="card p-4">
          <div class="d-flex align-items-center gap-3 mb-3">
            <div class="avatar"><?php echo strtoupper(substr($cv->getTitrePoste(), 0, 2)); ?></div>
            <div>
              <div class="fw-bold" style="font-size:.88rem;color:#1d2b4f"><?php echo htmlspecialchars($cv->getTitrePoste()); ?></div>
              <div style="font-size:.75rem;color:#8899bb"><?php echo htmlspecialchars($cv->getEmail()); ?></div>
            </div>
          </div>
          <?php if ($cv->getDescription()): ?>
            <p style="font-size:.8rem;color:#666;margin:0 0 12px;"><?php echo htmlspecialchars(mb_substr($cv->getDescription(), 0, 80)); ?>…</p>
          <?php endif; ?>
          <div class="d-flex gap-2 flex-wrap mb-3">
            ...
            <?php if ($cv->getTelephone()): ?><span style="font-size:.75rem;color:#8899bb"><i class="fas fa-phone me-1" style="color:#2a9d8f"></i><?php echo htmlspecialchars($cv->getTelephone()); ?></span><?php endif; ?>
          </div>
          <a href="view/frontoffice/show_cv.php?id=<?php echo $cv->getId(); ?>" class="btn-hero d-block text-center" style="padding:8px;font-size:.82rem;">
    ...
    <?php else: ?>
      <div class="row g-3">
        <?php foreach (array_slice($competences, 0, 6) as $comp):
          $b = match(strtolower($comp->getNiveau() ?? '')) {
        ...
        <div class="col-md-6 col-lg-4">
          <div class="card p-3 d-flex flex-row align-items-center gap-3">
            ...
            <div style="flex:1;min-width:0;">
              <div class="fw-bold" style="font-size:.88rem;color:#1d2b4f"><?php echo htmlspecialchars($comp->getNomCompetence()); ?></div>
              <div style="font-size:.75rem;color:#8899bb"><?php echo htmlspecialchars($comp->getCategorie() ?? ''); ?></div>
              <span class="<?php echo $b; ?>" style="display:inline-block;margin-top:4px;"><?php echo htmlspecialchars($comp->getNiveau() ?? 'N/A'); ?></span>
            </div>
          </div>
        </div>
        <?php endforeach; ?>
      ...
      <div class="col-md-6 col-lg-4">
        <div class="card p-4">
          <div class="d-flex align-items-start gap-3 mb-2">
            ...
            <div style="flex:1;min-width:0;">
              <div class="fw-bold" style="font-size:.88rem;color:#1d2b4f"><?php echo htmlspecialchars($exp->getTitre()); ?></div>
              <div style="font-size:.78rem;color:#457b9d;font-weight:600;"><?php echo htmlspecialchars($exp->getEntreprise() ?? ''); ?></div>
              <div style="font-size:.73rem;color:#8899bb;"><i class="fas fa-map-marker-alt me-1"></i><?php echo htmlspecialchars($exp->getLieu() ?? ''); ?></div>
            </div>
          </div>
          <div class="d-flex align-items-center gap-2 mb-2 flex-wrap">
            <span style="font-size:.72rem;color:#8899bb;">
              <i class="fas fa-calendar me-1"></i>
              <?php echo date('m/Y', strtotime($exp->getDateDebut())); ?> —
              <?php echo $exp->isEnCours() ? '<span style="color:#2a9d8f;font-weight:600">En cours</span>' : date('m/Y', strtotime($exp->getDateFin())); ?>
            </span>
            <span style="font-size:.7rem;font-weight:600;padding:2px 10px;border-radius:20px;background:rgba(69,123,157,.1);color:#457b9d;">
              <?php echo htmlspecialchars($exp->getTypeContrat()); ?>
            </span>
          </div>
          <?php if ($exp->getDescription()): ?>
            <p style="font-size:.78rem;color:#666;margin:0;"><?php echo htmlspecialchars(mb_substr($exp->getDescription(), 0, 90)); ?>…</p>
          <?php endif; ?>
        </div>
      </div>
      <?php endforeach; ?>
...
<section style="background:linear-gradient(135deg,#1d2b4f 0%,#0f1a36 100%);padding:60px 0;">
  <div class="container text-center text-white">
    <h3 class="fw-bold mb-2" style="font-size:1.6rem">Fonctionnalités Avancées</h3>
    <p style="color:#8899bb;font-size:.9rem;margin-bottom:36px">Découvrez les outils d'analyse de carrière basés sur vos compétences</p>
    <div class="row g-4 justify-content-center">
      <div class="col-md-5">
        <div style="background:rgba(230,57,70,.1);border:1px solid rgba(230,57,70,.25);border-radius:20px;padding:32px 28px">
          <div style="font-size:2.5rem;margin-bottom:14px">⭐</div>
          <h4 style="color:#e63946;font-weight:700;margin-bottom:10px">Recommandation par Score</h4>
          <p style="color:#8899bb;font-size:.85rem;line-height:1.6">Le système analyse les compétences du CV et calcule un score de compatibilité avec chaque métier disponible.</p>
          <a href="view/frontoffice/recommandation_metiers.php" class="btn-hero" style="margin-top:16px;display:inline-block">
            <i class="fas fa-star me-2"></i>Voir les recommandations
          </a>
        </div>
      </div>
      <div class="col-md-5">
        <div style="background:rgba(42,157,143,.1);border:1px solid rgba(42,157,143,.25);border-radius:20px;padding:32px 28px">
          <div style="font-size:2.5rem;margin-bottom:14px">🚀</div>
          <h4 style="color:#2a9d8f;font-weight:700;margin-bottom:10px">Évolution de Carrière</h4>
          <p style="color:#8899bb;font-size:.85rem;line-height:1.6">Le système trace le chemin d'évolution depuis le poste actuel vers les postes supérieurs avec les compétences à acquérir.</p>
          <a href="view/frontoffice/evolution_carriere.php" class="btn-hero" style="margin-top:16px;display:inline-block;background:linear-gradient(135deg,#2a9d8f,#264653)">
