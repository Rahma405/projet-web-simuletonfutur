...
if ($id <= 0) {
    $_SESSION['message'] = ['type' => 'danger', 'texte' => "Offre introuvable."];
    header('Location: list_offres.php');
    exit;
}

$offre = $ctrl->getById($id);
if (!$offre) {
    $_SESSION['message'] = ['type' => 'danger', 'texte' => "Offre introuvable."];
    header('Location: list_offres.php');
    exit;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erreurs)) {
        ...
        if ($ctrl->updateOffre($o)) {
            $_SESSION['message'] = ['type' => 'success', 'texte' => 'Offre modifiée avec succès.'];
            header('Location: list_offres.php');
            exit;
        }
        $erreurs['global'] = "Erreur lors de la modification de l'offre.";
    }
}
  ...
  <div class="col-lg-7">
    <div class="card">
      <div class="card-header">
        <i class="fas fa-pen me-2"></i>Modifier une offre d'emploi
      </div>
