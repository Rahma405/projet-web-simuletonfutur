...
$ctrl      = new MetierC();
$pageTitle = 'Modifier le Métier';
$id        = (int)($_GET['id'] ?? 0);
$metier    = $ctrl->getMetier($id);
$errors    = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $errors = $ctrl->edit($id, $_POST);
    if (empty($errors)) {
        $_SESSION['message'] = ['type' => 'success', 'texte' => 'Métier modifié avec succès.'];
        header('Location: list_metiers.php');
        exit;
    }
}
...
<div class="container py-5" style="max-width:700px">
  ...
  <div class="card p-4">
    <h4 class="fw-bold mb-4" style="color:#1d2b4f"><i class="fas fa-edit me-2" style="color:#e63946"></i>Modifier le métier</h4>
    ...
    <form method="POST" action="">
      ...
      <div class="mb-3">
        ...
        <input type="text" name="titre" class="form-control <?= isset($errors['titre']) ? 'is-invalid' : '' ?>"
               value="<?= htmlspecialchars($_POST['titre'] ?? $metier->getTitre()) ?>">
        <?php if (isset($errors['titre'])): ?><div class="invalid-feedback"><?= $errors['titre'] ?></div><?php endif; ?>
      </div>

      <div class="mb-3">
        <label class="form-label">Description</label>
        <textarea name="description" class="form-control" rows="3"><?= htmlspecialchars($_POST['description'] ?? $metier->getDescription()) ?></textarea>
      </div>

      <div class="mb-3">
        <label class="form-label">Secteur *</label>
        <input type="text" name="secteur" class="form-control <?= isset($errors['secteur']) ? 'is-invalid' : '' ?>"
               value="<?= htmlspecialchars($_POST['secteur'] ?? $metier->getSecteur()) ?>">
      ...
      <div class="row g-3 mb-4">
        <div class="col-6">
          ...
          <input type="number" name="salaire_min" class="form-control <?= isset($errors['salaire_min']) ? 'is-invalid' : '' ?>"
                 value="<?= htmlspecialchars($_POST['salaire_min'] ?? $metier->getSalaireMin()) ?>" min="0">
        ...
        <div class="col-6">
          ...
          <input type="number" name="salaire_max" class="form-control <?= isset($errors['salaire_max']) ? 'is-invalid' : '' ?>"
                 value="<?= htmlspecialchars($_POST['salaire_max'] ?? $metier->getSalaireMax()) ?>" min="0">
