...
class FrontModel extends Model {
    ...
    public function insertResult(int $quizId, int $userId, int $score, int $total): int {
        $stmt = $this->pdo->prepare(
            "INSERT INTO quiz_result (quiz_id, user_id, score, total, passed_at)
             VALUES (?, ?, ?, ?, NOW())"
        );
    ...
    public function getResultsByUser(int $userId): array {
        $stmt = $this->pdo->prepare(
            "SELECT r.id, r.quiz_id, r.score, r.total, r.passed_at,
                    q.type AS quiz_type, q.description AS quiz_description
             FROM quiz_result r
